...
> Web research underwent attempt (web_search, web_extract, browser_exec, open_preview) but
> remains unavailable in this environment (Firecrawl credit wall; cloud browser has no CDP endpoint),
> so external citations function as domain knowledge, not fetched evidence.
...
"7 hardening + 7 domain" split underwent once-assertion and falsification (subagent
verified count=14, all frontmatter valid, no dangling refs, but the 7/7 split
lacks backing by any artifact). Each skill encodes a control for one-or-more
clusters:
...
- (domain skills below function as orthogonal data-source capabilities, each with the
