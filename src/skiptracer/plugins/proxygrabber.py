...
def remove_proxy(fn, remline):
    """
    Removes a bad proxy from proxies.txt
    """
    f = open(fn, "r+")
    d = f.readlines()
    f.seek(0)
...
def write_file(d, fn):
    """
    used to write out files to disk
    """
    t = open(fn, "a")
    t.write(d)
    t.close()
...
def get_proxies():
    """
    Initial request to generate proxy list
    """
    print("\n[" + bc.CPRP + "?" + bc.CEND + "] " +
          bc.CCYN + "Proxy List Generator" + bc.CEND)
    ...
    print("  [" + bc.CRED + "X" + bc.CEND + "] " + bc.CYLW +
          "Testing proxies, please wait till complete..." + bc.CEND)
    return proxies
        ...
        for i in range(1, 11):  # Choose random 10 proxies and test from pool
            ...
            for xproto in ['http', 'https']:
                try:
                    ...
                    pass
        print("  [" + bc.CRED + "X" + bc.CEND + "] " + bc.CYLW +
              "Finished testing proxies, continue.\n" + bc.CEND)
        bi.proxy = new_proxy()
    return bi.proxy
