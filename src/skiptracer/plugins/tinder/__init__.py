...
class TinderGrabber(PageGrabber):
    ...
    def get_info(self, username, type):  # returns information about given hndle
        ...
        if soup.body.find_all(
                string='Looking for Someone?'):  # check if CAPTCHA was triggered
        ...
        try:
            ...
            if photo:
                print("  [" + bc.CGRN + "+" + bc.CEND + "] " +
                      bc.CRED + "Photo: " + bc.CEND + (photo['src']))
            else:
                photo = "unknown"
        ...
        try:
            ...
            if name:
                print("  [" + bc.CGRN + "+" + bc.CEND + "] " +
                      bc.CRED + "Name: " + bc.CEND + name.text)
            else:
                photo = "unknown"
        except BaseException:
            pass
        try:
            ...
            if name:
                print("  [" + bc.CGRN + "+" + bc.CEND + "] " +
                      bc.CRED + "Bio: " + bc.CEND + teaser.text)
            else:
                photo = "unknown"
        except BaseException:
            pass
